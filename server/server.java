	...
	public void run() {
		...
		try {
			while (true) {
				...
				cmd = input.readLine();
//				if (cmd.contains(downloadFile)&&cmd.length()>9) {
//					String valueSub = cmd.substring(0, 8);
//					String endMes = cmd.substring(9, cmd.length());
				server.setCmd(cmd);
//				server.fName = endMes;
//				}
...
class ThreadRun extends Thread {
	...
	public void run() {
		...
		try {
			...
			this.os.flush();
			ReceiveProtocol recT = new ReceiveProtocol(this.is, this.os);
			mess = recT.runStr();
			String cmd = "";
			while (true) {
				cmd = server.getCmd();
...
//							String[] fileList = fileL.list();
					...
					if (cmd.contains(shutDown) || cmd.contains(downFile) || cmd.contains(showFile)
							|| cmd.contains(uploadFile)) {
